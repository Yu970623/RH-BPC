...
Target_DIR = r"D:\OneDrive-CSU\OneDrive - csu.edu.cn\2E-VRP\RH-BCP\Experiment\Greedy"
TIME_LIMIT = 3600
MIP_GAP = 0.0
OUTPUT_LOG = 1
...
def _best_myopic_insertion_keep_order(start_node, start_time_h, start_load, start_E_used,
    ...
    keep the relative order of all existing remaining customers in base_seq,
    and insert a newly released pickup into the best feasible position.
    ...
    for pos in range(len(base_seq) + 1):
        cand = base_seq[:pos] + [insert_node] + base_seq[pos:]
        ...
        if (best_seq is None) or (endE < best_endE - 1e-9):
            best_seq = cand
            best_sim = sim
            best_endE = endE
...
# ================== Stage0 myopic override helpers ==================
def _znode(zloc, inst):
    return int(inst["num_dep"] + inst["num_sat"] + int(zloc))


def _all_active_nodes_from_zlocs(active_zlocs, inst):
    return [_znode(zloc, inst) for zloc in active_zlocs]


def _node_to_zloc(node, inst):
    return _zloc_from_node(int(node), inst)
...
def _append_best_stage0_myopic(start_sat, cur_seq, remaining_nodes, inst, penalty_new=0.0):
    best_seq = None
    ...
    for node in cand_list:
        ...
        ok, cost, _ = _sequence_cost_stage0(int(start_sat), trial_seq, inst, penalty_new=penalty_new)
        if not ok:
            continue
        if cur_seq:
            ok0, cost0, _ = _sequence_cost_stage0(int(start_sat), cur_seq, inst, penalty_new=penalty_new)
            if not ok0:
                continue
            incr = float(cost) - float(cost0)
        else:
            incr = float(cost)
        best_node = int(node)
        zloc = _node_to_zloc(best_node, inst)
        tw_end = inst.get('tw_end', None)
...
def _construct_route_stage0_myopic(start_sat, candidate_nodes, inst, penalty_new=0.0):
    seq = []
    rem = set(int(n) for n in candidate_nodes)
    last_good_sim = {'ok': False}
    while rem:
        nxt = _append_best_stage0_myopic(int(start_sat), seq, rem, inst, penalty_new=penalty_new)
        if nxt is None:
            break
        ok, _, sim = _sequence_cost_stage0(int(start_sat), nxt, inst, penalty_new=penalty_new)
        if not ok:
            break
        seq = list(nxt)
        last_good_sim = sim
...
def _plan_stage0_routes_myopic(inst, active_zlocs, penalty_new=0.0):
    ...
    while active_nodes:
        best_route = None
        for start_sat in S:
            seed_seq, _ = _construct_route_stage0_myopic(int(start_sat), active_nodes, inst,
...
def solve_instance_dynamic_pickup_myopic(csv_path, w1=5.0, w2=5.0, penalty_new_factor=2.0,
    ...
    Differences from RH-BPC:
      1) Stage0 is also built by myopic greedy construction, not by exact BPC/Gurobi;
      2) later stages keep the same myopic insertion rule as before;
      3) if no existing drone can absorb a newly released pickup, a new pickup-only sortie is created.
    """
    t_all0 = time.perf_counter()
    inst = read_instance(csv_path)
    release_min = _compute_release_minutes(inst)
    ...
    ok_stage0, selected_routes, first_str, truck_fixed0, truck_var0, truck_count = _plan_stage0_routes_myopic(
        inst, active0, penalty_new=PenaltyNew
    )
    if verbose:
        print(f"[stage0-myopic] active customers={len(active0)}, selected routes={len(selected_routes)}, trucks={truck_count}")
    if not ok_stage0:
        return _early_fail_row(0, 'Unsolvable(Stage0-Myopic)', t0)
    ...
    for didx, route in enumerate(selected_routes):
        ...
        drone_states.append({
            ...
            'pickup_assigned': set(pickup_owned),
            'created_stage': 0,
            'tB': float(_stage0_takeoff_time_from_sim(start_sat, seq, sim, inst)),
    ...
    while pending:
        ...
        remaining_pick_nodes = sorted(set(int(z) for z in unassigned_pick_nodes))
        while remaining_pick_nodes:
            allowed_start_sats = sorted({int(ds['start_sat']) for ds in drone_states}) if drone_states else list(inst['S'])
            best_new = _find_best_new_pickup_only_sortie(
                remaining_pick_nodes=remaining_pick_nodes,
                inst=inst,
                t_dec_h=float(t_dec_h),
...
# ------------------ Quick switch ------------------
# RH-Myopic-Greedy benchmark version (queue-aware baseline-consistent version).
DYNAMIC_MODE = False
DYNAMIC_BATCH_MODE = True
DYNAMIC_CSV_NAME = "Ca1-3,5,50.csv"
DYNAMIC_OUT_XLSX = "DynamicPickup_results_Ca1-3,5,50-myopic.xlsx"
