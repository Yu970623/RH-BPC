...
Target_DIR = r"D:\OneDrive-CSU\OneDrive - csu.edu.cn\2E-VRP\RH-BCP\Experiment\Repair"
TIME_LIMIT = 3600
MIP_GAP = 0.0
OUTPUT_LOG = 1
...
def _compute_release_minutes(inst):
    ...
    if tw_start is None or len(tw_start) == 0:
        return rel
    for zloc in range(num_cus):
        rel[zloc] = max(0.0, float(tw_start[zloc]) - 60.0)
    return rel
...
def solve_instance_dynamic_pickup(csv_path, w1=5.0, w2=5.0, penalty_new_factor=2.0,
    ...
    pool_cols = sum(len(routes_by_s[s]) for s in routes_by_s)
    if verbose:
        print(f"[stage0-pool] active customers={len(active0)}, pool_cols={pool_cols}")
    ...
    m.optimize()
    if m.Status not in (GRB.OPTIMAL, GRB.SUBOPTIMAL):
        return _early_fail_row(0, "Unsolvable(LP)", t0)
    lp_val = float(m.ObjVal)
    ...
    for set_id, name_list in all_sets:
        ...
        for name in name_list:
            ...
            for (w1, w2) in w_pairs:
                detail_xlsx = os.path.join(detail_dir, f"Dynamic_{name}_w{w1:g}-{w2:g}.xlsx")
...
def build_sets_for_dynamic():
    ...
    all_sets = [(3, Set3),(4, Set4)]
    return all_sets

def _best_myopic_insertion_keep_order(start_node, start_time_h, start_load, start_E_used,
                                      base_seq, insert_node, inst, energy_cap=DR['E']):
    """
    Repair/LNS-based local suffix operator on top of the correct Greedy rolling framework.

    Given an existing suffix ``base_seq`` and a newly released pickup ``insert_node``, this
    function keeps the same node set but *does not* force the original relative order to stay
    unchanged. Instead, it:
      1) builds a feasible seed by best-position insertion,
      2) applies a lightweight destroy-repair search on this single route only,
      3) returns the best feasible repaired suffix.

    This changes only the *route-local suffix optimizer*; all prefix-freeze / state inheritance /
    pickup-owner logic remains exactly the same as the correct Greedy implementation.

    Returns (ok, best_seq, best_sim).
    """
    base_seq = [int(z) for z in (base_seq or [])]
    insert_node = int(insert_node)
    ground_wait_free = (not _is_customer(int(start_node), inst))

    # ---------- helper: best feasible insertion of one node into a partial sequence ----------
    def _best_insert_one(cur_seq, node_to_insert):
        best_seq_loc = None
        best_sim_loc = None
        best_key = None
        for pos in range(len(cur_seq) + 1):
            cand = cur_seq[:pos] + [int(node_to_insert)] + cur_seq[pos:]
            sim = _simulate_route_from_state(
                start_node=int(start_node),
                start_time_h=float(start_time_h),
                start_load=float(start_load),
                start_energy_used=float(start_E_used),
                seq_customers=cand,
                inst=inst,
                ground_wait_free=ground_wait_free,
                remaining_energy_cap=energy_cap
            )
            if not sim.get('ok', False):
                continue
            endE = float(sim.get('end_energy_used', float('inf')))
            endT = float(sim.get('end_time_h', float('inf')))
            key = (endE, endT, tuple(int(z) for z in cand))
            if best_key is None or key < best_key:
                best_key = key
                best_seq_loc = [int(z) for z in cand]
                best_sim_loc = sim
        if best_seq_loc is None:
            return False, [], {'ok': False}
        return True, best_seq_loc, best_sim_loc

    # ---------- seed by best-position insertion ----------
    ok_seed, seed_seq, seed_sim = _best_insert_one(base_seq, insert_node)
    if not ok_seed:
        # robust exact fallback on the same node set
        nodes_set = set(base_seq) | {insert_node}
        ok_fb, seq_fb, sim_fb = _build_best_suffix_order(
            start_node=int(start_node),
            start_time_h=float(start_time_h),
            start_load=float(start_load),
            start_E_used=float(start_E_used),
            nodes_set=nodes_set,
            inst=inst,
            energy_cap=energy_cap,
            max_enum=200000,
            ground_wait_free_first=ground_wait_free
        )
        if ok_fb:
            return True, [int(z) for z in seq_fb], sim_fb
        return False, [], {'ok': False}

    best_seq = [int(z) for z in seed_seq]
    best_sim = seed_sim
    best_key = (float(best_sim.get('end_energy_used', float('inf'))),
                float(best_sim.get('end_time_h', float('inf'))),
                tuple(int(z) for z in best_seq))

    # ---------- lightweight route-local destroy-repair ----------
    rng = random.Random(20260417 + len(base_seq) + int(insert_node))
    all_nodes = [int(z) for z in best_seq]
    n = len(all_nodes)
    if n <= 1:
        return True, best_seq, best_sim

    lns_iters = min(30, max(8, 4 * n))
    for _ in range(lns_iters):
        cur = [int(z) for z in best_seq]
        q = max(1, int(round(0.25 * len(cur))))
        q = min(q, max(1, len(cur) - 1))
        rem_idx = sorted(rng.sample(range(len(cur)), q))
        removed = [cur[i] for i in rem_idx]
        kept = [z for i, z in enumerate(cur) if i not in set(rem_idx)]
        rng.shuffle(removed)

        feasible = True
        cur_seq = [int(z) for z in kept]
        cur_sim = None
        for zz in removed:
            ok_ins, seq_ins, sim_ins = _best_insert_one(cur_seq, zz)
            if not ok_ins:
                feasible = False
                break
            cur_seq = [int(z) for z in seq_ins]
            cur_sim = sim_ins
        if not feasible or cur_sim is None or not cur_sim.get('ok', False):
            continue

        key = (float(cur_sim.get('end_energy_used', float('inf'))),
               float(cur_sim.get('end_time_h', float('inf'))),
               tuple(int(z) for z in cur_seq))
        if key < best_key:
            best_key = key
            best_seq = [int(z) for z in cur_seq]
            best_sim = cur_sim

    return True, best_seq, best_sim
def _best_new_single_pickup_route(z_node, t_dec_h, inst, energy_cap=DR['E']):
    """
    Greedy baseline helper:
    for one unassigned pickup, create a single-pickup new sortie from the best satellite.

    Returns (ok, start_sat, seq, sim).
    """
    z_node = int(z_node)
    best = None  # (endE, s0, seq, sim)
    for s0 in inst['S']:
        sim = _simulate_route_from_state(
            start_node=int(s0),
            start_time_h=float(t_dec_h),
            start_load=0.0,
            start_energy_used=0.0,
            seq_customers=[z_node],
            inst=inst,
            ground_wait_free=True,
            remaining_energy_cap=energy_cap
        )
        if not sim.get('ok', False):
            continue
        endE = float(sim.get('end_energy_used', float('inf')))
        if (best is None) or (endE < best[0] - 1e-9):
            best = (endE, int(s0), [z_node], sim)
    if best is None:
        return False, None, [], {'ok': False}
    return True, best[1], best[2], best[3]


def solve_instance_dynamic_pickup_myopic(csv_path, w1=5.0, w2=5.0, penalty_new_factor=2.0,
                                         max_suffix_enum=200000, verbose=True,
                                         merge_release_window_min=3.0):
    """
    RH-Myopic-Greedy baseline.

    Main differences from the original dynamic solver:
      1) newly released pickups are assigned greedily, one by one;
      2) existing remaining order is preserved; only the new pickup is inserted into the best feasible position;
      3) if no existing drone can absorb the pickup, a new single-pickup sortie is created.

    Stage-0 exact solve is kept unchanged for fairness and code comparability.
    """
    t_all0 = time.perf_counter()
    inst = read_instance(csv_path)
    ...
    miss = warn_uncovered_customers_active(inst0, cover_mat, routes_by_s, active0)
    if miss:
        return [{"file": os.path.basename(csv_path), "stage": 0, "status": "Unsolvable(pool)"}]
    ...
    m.optimize()
    if m.Status not in (GRB.OPTIMAL, GRB.SUBOPTIMAL):
        return [{"file": os.path.basename(csv_path), "stage": 0, "status": "Unsolvable(LP)"}]
    ...
    if m.SolCount == 0 or m.Status in (GRB.INFEASIBLE, GRB.INF_OR_UNBD):
        return [{"file": os.path.basename(csv_path), "stage": 0, "status": "Unsolvable(MIP)"}]
    ...
    for didx, d in enumerate(drones):
        start_sat = int(d['start_sat'])
        seq = [int(z) for z in d['seq']]
        sim = _simulate_full_route_from_sat(start_sat, seq, inst, start_time_h=float(d.get('tD', 0.0)), ground_wait_free_first=False)
        if not sim.get('ok', False):
            return [{"file": os.path.basename(csv_path), "stage": 0, "status": f"Stage0RouteSimFail(d{didx})"}]
        ...
        drone_states.append({
            ...
            'timeline': tl,
            'deliv_owned': deliv_owned,
            'pickup_assigned': set(pickup_owned),
            'created_stage': 0,
            'tB': float(d.get('tB', 0.0)),
            'tD': float(d.get('tD', 0.0)),
            'end_energy_used': float(sim.get('end_energy_used', 0.0)),
            'end_time_h': float(sim.get('end_time_h', 0.0)),
            'end_sat': int(sim.get('end_sat', start_sat)),
            'last_cus_end_h': float(tl[-1]['t_end_h']) if tl else 0.0,
    ...
    while pending:
        ...
        for ds in drone_states:
            ...
            old_plan_seq = [int(z) for z in ds.get('plan_seq', [])]
            old_remaining_seq = [int(z) for z in old_plan_seq if int(z) not in served_nodes]
            ...
            drone_tmp.append({
                ...
                'cur_load': cur_load,
                'cur_E': cur_E,
                'old_remaining_seq': old_remaining_seq,
            })
        ...
        for info in drone_tmp:
            ...
            suffix_seq = [int(z) for z in info['old_remaining_seq'] if int(z) in (rem_deliv | rem_pick)]
            per_drone_state.append({
                'id': did,
                'created_stage': ds.get('created_stage', 0),
                'start_sat': int(ds['start_sat']),
        ...
        for z_node in pick_nodes_to_assign:
            z_node = int(z_node)
            best_choice = None  # (deltaE, idx, seq_ins, sim_ins)
            for i, st in enumerate(per_drone_state):
                ok, seq_ins, sim_ins = _best_myopic_insertion_keep_order(
                    start_node=st['cur_node'],
                    start_time_h=st['cur_time_h'],
                    start_load=st['cur_load'],
                    start_E_used=st['cur_E'],
                    base_seq=st['suffix_seq'],
                    insert_node=z_node,
                    inst=inst,
                    energy_cap=DR['E']
                )
                if not ok:
                    continue
                deltaE = float(sim_ins.get('end_energy_used', 0.0)) - float(st['cur_E'])
                if (best_choice is None) or (deltaE < best_choice[0] - 1e-9):
                    best_choice = (deltaE, i, seq_ins, sim_ins)

            if best_choice is None:
                unassigned_pick_nodes.append(z_node)
            else:
                _, i, seq_ins, sim_ins = best_choice
                per_drone_state[i]['suffix_seq'] = [int(z) for z in seq_ins]
                per_drone_state[i]['suffix_sim_cached'] = sim_ins
                per_drone_state[i]['rem_pick_nodes'].add(z_node)
                per_drone_state[i]['got_new_pickups'] = True
                committed_pick_nodes.add(int(z_node))
                try:
        ...
        total_energy_end = 0.0
        for st in per_drone_state:
            best_seq = [int(z) for z in st.get('suffix_seq', [])]
            best_sim = st.get('suffix_sim_cached', None)
            if best_sim is None:
                best_sim = _simulate_route_from_state(
                    start_node=st['cur_node'],
                    start_time_h=st['cur_time_h'],
                    start_load=st['cur_load'],
                    start_energy_used=st['cur_E'],
                    seq_customers=best_seq,
                    inst=inst,
                    ground_wait_free=(not _is_customer(int(st['cur_node']), inst)),
                    remaining_energy_cap=DR['E']
                )

            # emergency fallback for robustness only
            if not best_sim.get('ok', False):
                nodes_set = set(best_seq)
                ok_fb, seq_fb, sim_fb = _build_best_suffix_order(
                    start_node=st['cur_node'],
                    start_time_h=st['cur_time_h'],
                    start_load=st['cur_load'],
                ...
                if ok_fb:
                    best_seq = [int(z) for z in seq_fb]
                    best_sim = sim_fb
                else:
                    best_seq = []
                    best_sim = {'ok': False, 'timeline': []}

            full_seq = st['prefix_seq'] + best_seq

            if best_sim.get('ok', False):
                end_sat = int(best_sim.get('end_sat', _nearest_sat_from(full_seq[-1], inst)[0] if full_seq else st['start_sat']))
                E_end = float(best_sim.get('end_energy_used', st['cur_E']))
                suffix_tl = best_sim.get('timeline', [])
                timeline_full = st['prefix_timeline'] + suffix_tl
                end_time_h = float(best_sim.get('end_time_h', st['cur_time_h']))
                last_cus_end_h = float(timeline_full[-1]['t_end_h']) if timeline_full else float(st['cur_time_h'])
            else:
                end_sat = int(st['start_sat'])
                E_end = float(st['cur_E'])
                timeline_full = list(st['prefix_timeline'])
                end_time_h = float(st['cur_time_h'])
                last_cus_end_h = float(timeline_full[-1]['t_end_h']) if timeline_full else float(st['cur_time_h'])
        ...
        # If unassigned pickups remain, dispatch as many new pickup-only sorties as needed.
        # To keep the same dispatch envelope as the main RH-BPC framework, the new-sortie
        # generation rule is shared; the myopic benchmark differs only in how existing routes
        # absorb newly released pickups.
        new_drones_created = 0
        new_routes_str = []
        stage_abort = False
        remaining_pick_nodes = sorted(set(int(z) for z in unassigned_pick_nodes))
        ...
        for idx in range(len(drone_states)):
            ...
            if did in upd_by_id:
                u = upd_by_id[did]
                drone_states[idx]['plan_seq'] = u['full_seq']
                drone_states[idx]['pickup_assigned'] = set([int(_n) for _n in drone_states[idx]['plan_seq'] if _is_pick_node(_n)])
                drone_states[idx]['timeline'] = u['timeline']
                drone_states[idx]['end_energy_used'] = float(u.get('end_energy_used', drone_states[idx].get('end_energy_used', 0.0)))
                drone_states[idx]['end_time_h'] = float(u.get('end_time_h', drone_states[idx].get('end_time_h', 0.0)))
                drone_states[idx]['end_sat'] = int(u.get('end_sat', drone_states[idx].get('end_sat', drone_states[idx].get('start_sat', 0))))
                drone_states[idx]['last_cus_end_h'] = float(u.get('last_cus_end_h', drone_states[idx].get('last_cus_end_h', 0.0)))
        ...
        drone_fixed = float(h2) * len(drone_states) + float(PenaltyNew) * sum(1 for d in drone_states if d['created_stage'] > 0)
        total_energy_stage = sum(float(d.get('end_energy_used', 0.0)) for d in drone_states)
        drone_var = float(c_u) * float(total_energy_stage)
        obj1_stage = float(truck_fixed0) + float(drone_fixed)
        obj2_stage = float(truck_var0) + float(drone_var)
    ...
    if verbose:
        print(f"[done-myopic] stages={len(stage_rows)}, total wall time={time.perf_counter() - t_all0:.2f}s")

    return stage_rows


def run_dynamic_single_myopic(csv_path, out_xlsx=None, w1=5.0, w2=5.0, penalty_new_factor=2.0,
                              max_suffix_enum=200000):
    rows = solve_instance_dynamic_pickup_myopic(
        csv_path=csv_path, w1=w1, w2=w2,
        penalty_new_factor=penalty_new_factor,
        max_suffix_enum=max_suffix_enum,
        verbose=True
...
def run_dynamic_single(csv_path, out_xlsx=None, w1=5.0, w2=5.0, penalty_new_factor=2.0,
                       max_suffix_enum=200000, lock_unaffected=True):
    """Public single-instance entry for the RH-Myopic-Greedy benchmark."""
    rows = solve_instance_dynamic_pickup_myopic(
        csv_path=csv_path, w1=w1, w2=w2,
        penalty_new_factor=penalty_new_factor,
        max_suffix_enum=max_suffix_enum,
...
def run_dynamic_batch(all_sets, detail_dir_name="DynamicPickup_details_myopic",
                      w_pairs=None, penalty_new_factor=2.0, max_suffix_enum=200000,
                      lock_unaffected=True):
    """Batch entry aligned with the original RH-BPC interface."""
    if w_pairs is None:
        w_pairs = [(5.0, 5.0)]
    ...
    for set_id, name_list in all_sets:
        ...
        for name in name_list:
            csv_path = os.path.join(BASE_DIR, name + ".csv")
            for (w1, w2) in w_pairs:
                detail_xlsx = os.path.join(detail_dir, f"Dynamic_{name}_Greedy.xlsx")
                try:
                    df_detail = run_dynamic_single(
                        csv_path,
                        out_xlsx=detail_xlsx,
        ...
        df_sum = pd.DataFrame(summary_rows)
        out_xlsx = os.path.join(Target_DIR, f"DynamicPickup_Set{set_id}_results_myopic.xlsx")
        safe_save_table(df_sum, out_xlsx)


# ================== Repair/LNS override block ==================
def _all_active_nodes_from_zlocs(active_zlocs, inst):
    return [int(inst['num_dep'] + inst['num_sat'] + int(z)) for z in active_zlocs]


def _node_to_zloc(node, inst):
    return int(node) - int(inst['num_dep']) - int(inst['num_sat'])


def _sequence_delivery_load(seq_nodes, inst):
    total = 0
    for z in seq_nodes:
        zloc = _node_to_zloc(int(z), inst)
        total += int(inst['Dd'][zloc])
    return int(total)


def _sequence_cost_stage0(start_sat, seq_nodes, inst, penalty_new=0.0):
    sim = _simulate_full_route_from_sat(
        int(start_sat), [int(z) for z in seq_nodes], inst,
        start_time_h=0.0, ground_wait_free_first=True
    )
    if not sim.get('ok', False):
        return False, float('inf'), sim
    delivery_load = _sequence_delivery_load(seq_nodes, inst)
    fixed_cost = float(h2) + (float(penalty_new) if delivery_load == 0 else 0.0)
    variable_cost = float(c_u) * float(sim.get('end_energy_used', 0.0))
    return True, float(fixed_cost + variable_cost), sim


def _stage0_takeoff_time_from_sim(start_sat, seq_nodes, sim, inst):
    if not seq_nodes:
        return 0.0
    tl = sim.get('timeline', []) if isinstance(sim, dict) else []
    if not tl:
        return 0.0
    z0 = int(seq_nodes[0])
    fly0 = float(inst['dist'][int(start_sat), z0]) / max(1e-9, DR['f_s'])
    t_arr0 = float(tl[0].get('t_arrive_h', 0.0))
    return max(0.0, t_arr0 - fly0)


def _first_echelon_direct_trips(selected_routes, inst):
    P = inst['P']
    S = inst['S']
    dist = inst['dist']
    num_dep = inst['num_dep']
    if not P:
        return '', 0.0, 0.0, 0
    sat_delivery = defaultdict(float)
    for route in selected_routes:
        s = int(route['start_sat'])
        sat_delivery[s] += float(_sequence_delivery_load(route.get('seq', []), inst))
    trips = []
    truck_fixed_cost = 0.0
    truck_variable_cost = 0.0

    def _node_local_12(node):
        ps_nodes = list(range(num_dep + inst['num_sat']))
        return ps_nodes.index(int(node)) + 1

    for s in S:
        demand = float(sat_delivery.get(int(s), 0.0))
        if demand <= 1e-12:
            continue
        remaining = demand
        while remaining > 1e-12:
            best_depot = min(P, key=lambda p: float(dist[int(p), int(s)]))
            truck_fixed_cost += float(h1)
            truck_variable_cost += float(c_v) * (
                float(dist[int(best_depot), int(s)]) + float(dist[int(s), int(best_depot)])
            )
            trips.append(f"{_node_local_12(best_depot)} {_node_local_12(s)} {_node_local_12(best_depot)}")
            remaining -= float(Q1)

    return ' | '.join(trips), float(truck_fixed_cost), float(truck_variable_cost), len(trips)


def _stage0_sort_key(node, inst):
    zloc = _node_to_zloc(int(node), inst)
    tw_start = inst.get('tw_start', None)
    tw_end = inst.get('tw_end', None)
    e = float(tw_start[zloc]) if tw_start is not None and len(tw_start) > 0 else 0.0
    l = float(tw_end[zloc]) if tw_end is not None and len(tw_end) > 0 else 1e18
    typ = 0 if int(inst['Dd'][zloc]) > 0 else 1
    return (e, l, typ, int(zloc))


def _best_stage0_repair_insertion(start_sat, base_seq, insert_node, inst, penalty_new=0.0):
    base_seq = [int(z) for z in (base_seq or [])]
    insert_node = int(insert_node)

    def _best_insert_one(cur_seq, node_to_insert):
        best_seq_loc, best_sim_loc, best_key = None, None, None
        for pos in range(len(cur_seq) + 1):
            cand = cur_seq[:pos] + [int(node_to_insert)] + cur_seq[pos:]
            ok, _, sim = _sequence_cost_stage0(int(start_sat), cand, inst, penalty_new=penalty_new)
            if not ok:
                continue
            endE = float(sim.get('end_energy_used', float('inf')))
            endT = float(sim.get('end_time_h', float('inf')))
    ...
    if not ok_seed:
        nodes_set = set(base_seq) | {insert_node}
        start_load = float(sum(int(inst['Dd'][_node_to_zloc(z, inst)]) for z in nodes_set))
        ok_fb, seq_fb, sim_fb = _build_best_suffix_order(
            start_node=int(start_sat),
            start_time_h=0.0,
    ...
    rng = random.Random(20260420 + len(base_seq) + int(insert_node))
    n = len(best_seq)
    if n <= 1:
        return True, best_seq, best_sim
    ...
    for _ in range(lns_iters):
        ...
        rem_set = set(rem_idx)
        removed = [cur[i] for i in rem_idx]
        kept = [z for i, z in enumerate(cur) if i not in rem_set]
        rng.shuffle(removed)
...
def _construct_route_stage0_repair(start_sat, candidate_nodes, inst, penalty_new=0.0):
    seq = []
    rem = set(int(n) for n in candidate_nodes)
    last_good_sim = {'ok': False}
    while rem:
        best = None
        cand_list = list(rem)
        cand_list.sort(key=lambda n: (_stage0_sort_key(n, inst), float(inst['dist'][int(start_sat), int(n)])))
        for node in cand_list:
            ok_ins, seq_ins, sim_ins = _best_stage0_repair_insertion(int(start_sat), seq, int(node), inst,
                                                                      penalty_new=penalty_new)
            if not ok_ins:
                continue
            ok1, cost1, _ = _sequence_cost_stage0(int(start_sat), seq_ins, inst, penalty_new=penalty_new)
            if not ok1:
                continue
            if seq:
                ok0, cost0, _ = _sequence_cost_stage0(int(start_sat), seq, inst, penalty_new=penalty_new)
                if not ok0:
                    continue
                incr = float(cost1) - float(cost0)
            else:
                incr = float(cost1)
            zloc = _node_to_zloc(int(node), inst)
            tw_end = inst.get('tw_end', None)
            due = float(tw_end[zloc]) if tw_end is not None and len(tw_end) > 0 else 1e18
            key = (float(incr), float(due), tuple(int(z) for z in seq_ins))
            if best is None or key < best['key']:
                best = {'node': int(node), 'seq': [int(z) for z in seq_ins], 'sim': sim_ins, 'key': key}
        if best is None:
            break
        seq = list(best['seq'])
        last_good_sim = best['sim']
        rem.remove(int(best['node']))
    return seq, last_good_sim


def _plan_stage0_routes_repair(inst, active_zlocs, penalty_new=0.0):
    active_nodes = set(_all_active_nodes_from_zlocs(active_zlocs, inst))
    S = inst['S']
    selected_routes = []
    if not active_nodes:
        first_str, truck_fixed, truck_var, truck_count = _first_echelon_direct_trips([], inst)
        return True, [], first_str, truck_fixed, truck_var, truck_count

    while active_nodes:
        best_route = None
        for start_sat in S:
            seed_seq, seed_sim = _construct_route_stage0_repair(int(start_sat), active_nodes, inst,
                                                                penalty_new=penalty_new)
            if not seed_seq:
                continue
            ok, route_cost, sim = _sequence_cost_stage0(int(start_sat), seed_seq, inst, penalty_new=penalty_new)
            if not ok:
                continue
            cov = max(1, len(seed_seq))
            score = float(route_cost) / float(cov)
            route_info = {
                'start_sat': int(start_sat),
                'seq': [int(z) for z in seed_seq],
                'sim': sim if sim.get('ok', False) else seed_sim,
                'route_cost': float(route_cost),
                'score': float(score),
            }
            if (best_route is None or
                    len(seed_seq) > len(best_route['seq']) or
                    (len(seed_seq) == len(best_route['seq']) and score < best_route['score'] - 1e-9)):
                best_route = route_info
        if best_route is None:
            return False, [], '', 0.0, 0.0, 0
        selected_routes.append(best_route)
        active_nodes -= set(best_route['seq'])

    first_str, truck_fixed, truck_var, truck_count = _first_echelon_direct_trips(selected_routes, inst)
    return True, selected_routes, first_str, truck_fixed, truck_var, truck_count


# Override the route-local insertion operator used by later stages.
def _best_myopic_insertion_keep_order(start_node, start_time_h, start_load, start_E_used,
                                      base_seq, insert_node, inst, energy_cap=DR['E']):
    """
    Repair/LNS-based local suffix operator.
    It starts from best-position insertion and then applies a small destroy-repair search
    on the single route while keeping the same node set.
    """
    base_seq = [int(z) for z in (base_seq or [])]
    insert_node = int(insert_node)
    ground_wait_free = (not _is_customer(int(start_node), inst))
    ...
    def _best_insert_one(cur_seq, node_to_insert):
        ...
        for pos in range(len(cur_seq) + 1):
            ...
            if not sim.get('ok', False):
                continue
            key = (
                float(sim.get('end_energy_used', float('inf'))),
                float(sim.get('end_time_h', float('inf'))),
                tuple(int(z) for z in cand)
            )
            if best_key is None or key < best_key:
                best_key = key
                best_seq_loc = [int(z) for z in cand]
    ...
    best_sim = seed_sim
    best_key = (
        float(best_sim.get('end_energy_used', float('inf'))),
        float(best_sim.get('end_time_h', float('inf'))),
        tuple(int(z) for z in best_seq)
    )
    ...
    for _ in range(lns_iters):
        ...
        if not feasible or cur_sim is None or not cur_sim.get('ok', False):
            continue
        key = (
            float(cur_sim.get('end_energy_used', float('inf'))),
            float(cur_sim.get('end_time_h', float('inf'))),
            tuple(int(z) for z in cur_seq)
        )
        if key < best_key:
...
def solve_instance_dynamic_pickup_repair(csv_path, w1=5.0, w2=5.0, penalty_new_factor=2.0,
                                         max_suffix_enum=200000, verbose=True,
                                         merge_release_window_min=3.0,
                                         lock_unaffected=True):
    """
    RH-Repair-on-Greedy-Framework baseline.

    Characteristics:
      1) Stage0 is built by repair/LNS route construction, not by RH-BPC exact solve;
      2) later stages keep prefix-freeze / state inheritance / pickup-owner persistence;
      3) when a route needs to absorb new pickups, a route-local repair/LNS operator is used.
    """
    inst = read_instance(csv_path)
    release_min = _compute_release_minutes(inst)
    PenaltyNew = float(penalty_new_factor) * float(h2)

    def _early_fail_row(stage_id, status, t_start, **extra):
        row = {
            'Instances': os.path.basename(csv_path),
            'Stage': int(stage_id),
            'TriggerRelease(min)': '',
    ...
    active0 = _active_zlocs_stage0(inst, release_min)
    t0 = time.perf_counter()
    ok_stage0, selected_routes, first_str, truck_fixed0, truck_var0, truck_count = _plan_stage0_routes_repair(
        inst, active0, penalty_new=PenaltyNew
    )
    if verbose:
        print(f"[stage0-repair] active customers={len(active0)}, selected routes={len(selected_routes)}, trucks={truck_count}")
    if not ok_stage0:
        return _early_fail_row(0, 'Unsolvable(Stage0-Repair)', t0)

    Dd, Dp = inst['Dd'], inst['Dp']
    drone_states = []
    second_routes0 = []
    for didx, route in enumerate(selected_routes):
        start_sat = int(route['start_sat'])
        seq = [int(z) for z in route['seq']]
        sim = route.get('sim', None)
        if sim is None or not sim.get('ok', False):
            sim = _simulate_full_route_from_sat(start_sat, seq, inst, start_time_h=0.0, ground_wait_free_first=True)
        if not sim.get('ok', False):
            return _early_fail_row(0, f'Stage0RouteSimFail(d{didx})', t0)
        end_sat0 = int(sim.get('end_sat', _nearest_sat_from(seq[-1], inst)[0] if seq else start_sat))
        second_routes0.append(_format_route_12(start_sat, seq, end_sat0, inst))
        deliv_owned, pickup_owned = set(), set()
        for z in seq:
            zloc = _zloc_from_node(z, inst)
            if int(Dd[zloc]) > 0:
                deliv_owned.add(int(z))
            if int(Dp[zloc]) > 0:
                pickup_owned.add(int(z))
        tl = sim.get('timeline', [])
        td = float(_stage0_takeoff_time_from_sim(start_sat, seq, sim, inst))
        drone_states.append({
            'id': didx,
            'start_sat': start_sat,
            'prefix_seq': [],
    ...
    obj1_0 = float(truck_fixed0)
    obj2_0 = float(truck_var0)
    for route in selected_routes:
        seq = [int(z) for z in route['seq']]
        sim = route.get('sim', {})
        obj1_0 += float(h2)
        if _sequence_delivery_load(seq, inst) == 0:
            obj1_0 += float(PenaltyNew)
        obj2_0 += float(c_u) * float(sim.get('end_energy_used', 0.0))
    ...
    def _next_global_decision_time_h(t_rel_h):
        ...
        for ds in drone_states:
            ...
            if decided is None:
                decided = float(t_rel_h)
            times.append(float(decided))
        return float(min(times)) if times else float(t_rel_h)

    def _pick_sort_key(n):
        z = _zloc_from_node(int(n), inst)
        tws = float(inst.get('tw_start', [0]*inst['num_cus'])[z]) if inst.get('tw_start', None) is not None else 0.0
        return (float(release_min[z]), tws, int(z))
    ...
    while pending:
        stage_id += 1
        next_rel_min, _ = pending[0]
        target_rel_min = float(next_rel_min) + (float(merge_release_window_min) if float(merge_release_window_min) > 1e-9 else 0.0)
        t_rel_h = float(target_rel_min) / 60.0
        t_dec_h = _next_global_decision_time_h(t_rel_h)
        t_dec_min = int(round(t_dec_h * 60.0))
        ...
        while pending and pending[0][0] <= t_dec_min_f + 1e-9:
            ...
            new_pickups_zloc.append(int(zloc))
        if verbose:
            print(f"[stage{stage_id}-repair] t_dec={t_dec_min} min, activate pickups zloc={new_pickups_zloc}")
        ...
        for ds in drone_states:
            ...
            if di < 0:
                ...
                prefix_tl = []
                cur_node = int(ds['start_sat'])
                cur_time = max(float(t_dec_h), float(ds.get('tD', t_dec_h)))
                cur_E = 0.0
                cur_load = 0.0
                for z in ds.get('deliv_owned', set()):
                    zloc = _zloc_from_node(int(z), inst)
                ...
                last = prefix_tl[-1]
                cur_node = int(last['node'])
                cur_time = max(float(last['t_end_h']), float(t_dec_h))
                cur_load = float(last['load_after'])
                cur_E = float(last['E_used'])
            served_nodes = set(prefix_seq)
            for _n in prefix_seq:
        ...
        for z_node in pick_nodes_to_assign:
            ...
            for i, st in enumerate(per_drone_state):
                ...
                old_seq = [int(z) for z in st.get('old_plan_seq', [])]
                served_now = set(int(z) for z in st.get('prefix_seq', []))
                base_seq = [int(z) for z in old_seq if (int(z) not in served_now) and (int(z) in nodes_set)]
                ok_ins, seq_ins, sim_ins = _best_myopic_insertion_keep_order(
                    start_node=st['cur_node'],
                    start_time_h=st['cur_time_h'],
                    start_load=st['cur_load'],
                    start_E_used=st['cur_E'],
                ...
                if not ok_ins:
                    continue
                deltaE = float(sim_ins.get('end_energy_used', 0.0)) - float(st['cur_E'])
                endT = float(sim_ins.get('end_time_h', float('inf')))
                key = (float(deltaE), float(endT), i)
                if best_choice is None or key < best_choice[0]:
                    best_choice = (key, i, seq_ins, sim_ins)
            if best_choice is None:
                unassigned_pick_nodes.append(z_node)
            else:
                _, i, _, _ = best_choice
        ...
        for st in per_drone_state:
            ...
            if not ok_plan:
                extra_new = [int(z) for z in sorted(set(st['rem_pick_nodes']) - set(keep_suffix), key=_pick_sort_key)]
                cur_seq = list(keep_suffix)
                cur_sim = None
                feasible = True
                for zz in extra_new:
                    ok_ins, seq_ins, sim_ins = _best_myopic_insertion_keep_order(
                ...
                if feasible:
                    if cur_sim is None:
                        cur_sim = _simulate_route_from_state(
                            start_node=st['cur_node'],
                            start_time_h=st['cur_time_h'],
                            start_load=st['cur_load'],
                            start_energy_used=st['cur_E'],
                            seq_customers=cur_seq,
                            inst=inst,
                            ground_wait_free=(not _is_customer(int(st['cur_node']), inst)),
                            remaining_energy_cap=DR['E']
                        )
                    if cur_sim.get('ok', False):
                        best_seq = [int(z) for z in cur_seq]
                        best_sim = cur_sim
                        ok_plan = True
            ...
            full_seq = st['prefix_seq'] + list(best_seq)
            if best_sim.get('ok', False):
                end_sat = int(best_sim.get('end_sat', _nearest_sat_from(full_seq[-1], inst)[0] if full_seq else st['start_sat']))
                E_end = float(best_sim.get('end_energy_used', st['cur_E']))
                suffix_tl = best_sim.get('timeline', [])
        ...
        new_drones_created = 0
        new_routes_str = []
        remaining_pick_nodes = sorted(set(int(z) for z in unassigned_pick_nodes), key=_pick_sort_key)
        while remaining_pick_nodes:
            z_pick = int(remaining_pick_nodes[0])
            ok_new, s0, seq_new, sim_new = _best_new_single_pickup_route(z_pick, float(t_dec_h), inst, energy_cap=DR['E'])
            if not ok_new:
                stage_rows.append({
                    'Instances': os.path.basename(csv_path),
                    'Stage': stage_id,
            ...
            drone_states.append({
                ...
                'start_sat': int(s0),
                'prefix_seq': [],
                'plan_seq': list(seq_new),
                'timeline': sim_new.get('timeline', []),
                'deliv_owned': set(),
                'pickup_assigned': {int(z_pick)},
                'created_stage': stage_id,
                'tB': float(t_dec_h),
                'tD': float(t_dec_h),
                'end_energy_used': float(sim_new.get('end_energy_used', 0.0)),
                'end_time_h': float(sim_new.get('end_time_h', t_dec_h)),
                'end_sat': int(sim_new.get('end_sat', s0)),
                'last_cus_end_h': float(sim_new.get('timeline', [])[-1].get('t_end_h', t_dec_h)) if sim_new.get('timeline', []) else float(t_dec_h),
            })
            new_drones_created += 1
            total_energy_end += float(sim_new.get('end_energy_used', 0.0))
            new_routes_str.append(_format_route_12(int(s0), [int(z) for z in seq_new], int(sim_new.get('end_sat', s0)), inst))
            remaining_pick_nodes = [int(z) for z in remaining_pick_nodes if int(z) != z_pick]
        ...
        for idx in range(len(drone_states)):
            ...
            if did in upd_by_id:
                ...
                drone_states[idx]['pickup_assigned'] = set([int(_n) for _n in drone_states[idx]['plan_seq'] if _is_pick_node(_n)])
                drone_states[idx]['timeline'] = u['timeline']
                drone_states[idx]['end_energy_used'] = float(u['end_energy_used'])
                drone_states[idx]['end_time_h'] = float(u['end_time_h'])
                drone_states[idx]['end_sat'] = int(u['end_sat'])
                drone_states[idx]['last_cus_end_h'] = float(u['last_cus_end_h'])

        obj1_t = float(truck_fixed0)
        obj2_t = float(truck_var0)
        second_routes_t = []
        for ds in drone_states:
            seq = [int(z) for z in ds.get('plan_seq', [])]
            if not seq:
                continue
            obj1_t += float(h2)
            if _sequence_delivery_load(seq, inst) == 0:
                obj1_t += float(PenaltyNew)
            obj2_t += float(c_u) * float(ds.get('end_energy_used', 0.0))
            second_routes_t.append(_format_route_12(int(ds['start_sat']), seq, int(ds.get('end_sat', ds['start_sat'])), inst))
        ...
        stage_rows.append({
            ...
            'NewPickups(zloc)': ','.join(str(int(z) + 1) for z in new_pickups_zloc),
            'First-echelon': first_str,
            'Second-echelon': ' | '.join(second_routes_t),
            'DecisionTimes(min)': decision_times_str,
            'NumSorties': len([d for d in drone_states if d.get('plan_seq', [])]),
            'Obj1': round(obj1_t, 6),
            'Obj2': round(obj2_t, 6),
            'Sum_obj': round(obj1_t + obj2_t, 6),
            'PenaltyNew': round(PenaltyNew, 6),
            'NewSortieCreated': int(new_drones_created),
            'DroneEnergy(Wh)': round(sum(float(d.get('end_energy_used', 0.0)) for d in drone_states), 6),
            'ReplanSolveTime(s)': round(time.perf_counter() - t_stage_start, 3),
            'Status': 'OK'
        })
...
def run_dynamic_single_repair(csv_path, out_xlsx=None, w1=5.0, w2=5.0, penalty_new_factor=2.0,
                              max_suffix_enum=200000, lock_unaffected=True):
    rows = solve_instance_dynamic_pickup_repair(
        csv_path=csv_path, w1=w1, w2=w2,
        penalty_new_factor=penalty_new_factor,
        max_suffix_enum=max_suffix_enum,
        verbose=True,
...
def run_dynamic_single(csv_path, out_xlsx=None, w1=5.0, w2=5.0, penalty_new_factor=2.0,
                       max_suffix_enum=200000, lock_unaffected=True):
    return run_dynamic_single_repair(csv_path, out_xlsx, w1, w2, penalty_new_factor,
                                     max_suffix_enum, lock_unaffected)


def run_dynamic_batch(all_sets, detail_dir_name="DynamicPickup_details_repair",
                      w_pairs=None, penalty_new_factor=2.0, max_suffix_enum=200000,
                      lock_unaffected=True):
    if w_pairs is None:
        w_pairs = [(5.0, 5.0)]
    ...
    for set_id, name_list in all_sets:
        ...
        for name in name_list:
            csv_path = os.path.join(BASE_DIR, name + ".csv")
            for (w1, w2) in w_pairs:
                detail_xlsx = os.path.join(detail_dir, f"Dynamic_{name}_Repair.xlsx")
                try:
                    df_detail = run_dynamic_single(
                        csv_path,
                        out_xlsx=detail_xlsx,
        ...
        df_sum = pd.DataFrame(summary_rows)
        out_xlsx = os.path.join(Target_DIR, f"DynamicPickup_Set{set_id}_results_repair.xlsx")
        safe_save_table(df_sum, out_xlsx)


# ------------------ Quick switch (Repair/LNS) ------------------
DYNAMIC_MODE = True
DYNAMIC_BATCH_MODE = False
DYNAMIC_CSV_NAME = "Ca4-6,4,50.csv"
DYNAMIC_OUT_XLSX = "DynamicPickup_results_Ca4-6,4,50-repair.xlsx"
...
if __name__ == "__main__":
    if DYNAMIC_BATCH_MODE:
        ...
        out_xlsx = os.path.join(BASE_DIR, DYNAMIC_OUT_XLSX)
