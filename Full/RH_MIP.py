...
Target_DIR = r"D:\OneDrive-CSU\OneDrive - csu.edu.cn\2E-VRP\RH-BCP\Experiment\MIP"
TIME_LIMIT = 3600
MIP_GAP = 0.0
OUTPUT_LOG = 1
...
def _stage0_takeoff_time_from_sim(start_sat, seq_nodes, sim, inst):
    """Infer the actual takeoff time implied by a stage-0 simulation."""
    if not seq_nodes:
        return 0.0
...
def _first_echelon_direct_trips(selected_routes, inst):
    """
    Reconstruct a simple first-echelon plan from stage0 second-echelon routes.
    Aggregate delivery demand by start satellite and dispatch depot-satellite-depot trips.
    """
    P = inst['P']
    S = inst['S']
    dist = inst['dist']
...
def _stage0_sort_key(node, inst):
    zloc = _zloc_from_node(int(node), inst)
    tw_start = inst.get('tw_start', None)
    tw_end = inst.get('tw_end', None)
    ...
    typ = 0 if int(inst['Dd'][zloc]) > 0 else 1  # deliveries first under ties
    return (e, l, typ, int(zloc))


def _optimize_stage0_route_mip(start_sat, seq_nodes, inst, penalty_new=0.0, max_suffix_enum=200000):
    """
    Optimize one stage-0 route with the same Stage-MIP suffix solver used in later stages.
    """
    seq_nodes = [int(z) for z in seq_nodes]
    if not seq_nodes:
        return False, float('inf'), [], {'ok': False}

    start_load = _sequence_delivery_load(seq_nodes, inst)
    if float(start_load) > float(Q2) + 1e-9:
        return False, float('inf'), [], {'ok': False, 'reason': 'start_load_over_q2'}
    ...
    ok, seq_best, sim_best = _build_best_suffix_order(
        ...
        start_load=float(start_load),
        start_E_used=0.0,
        nodes_set=set(int(z) for z in seq_nodes),
        inst=inst,
        energy_cap=DR['E'],
        max_enum=max_suffix_enum,
        ground_wait_free_first=True
    ...
    if not ok:
        return False, float('inf'), [], sim_best if isinstance(sim_best, dict) else {'ok': False}

    delivery_load = _sequence_delivery_load(seq_best, inst)
    fixed_cost = float(h2) + (float(penalty_new) if delivery_load == 0 else 0.0)
    variable_cost = float(c_u) * float(sim_best.get('end_energy_used', 0.0))
    total_cost = float(fixed_cost + variable_cost)
    return True, total_cost, [int(z) for z in seq_best], sim_best


def _append_best_stage0_mip(start_sat, cur_seq, remaining_nodes, inst, penalty_new=0.0, max_suffix_enum=200000):
    best_seq = None
    best_score = float('inf')
    cand_list = list(remaining_nodes)
    cand_list.sort(key=lambda n: (_stage0_sort_key(n, inst), float(inst['dist'][int(start_sat), int(n)])))

    cur_cost = 0.0
    if cur_seq:
        ok0, cur_cost, _, _ = _optimize_stage0_route_mip(
            int(start_sat), cur_seq, inst, penalty_new=penalty_new, max_suffix_enum=max_suffix_enum
        )
        if not ok0:
            return None

    for node in cand_list:
        trial_seq = list(cur_seq) + [int(node)]
        ok, cost, _, _ = _optimize_stage0_route_mip(
            int(start_sat), trial_seq, inst, penalty_new=penalty_new, max_suffix_enum=max_suffix_enum
        )
        if not ok:
            continue
        incr = float(cost) - float(cur_cost) if cur_seq else float(cost)
        best_node = int(node)
        zloc = _zloc_from_node(best_node, inst)
        tw_end = inst.get('tw_end', None)
        due = float(tw_end[zloc]) if tw_end is not None and len(tw_end) > 0 else 1e18
        score = float(incr) + 0.0001 * float(due)
...
def _construct_route_stage0_mip(start_sat, candidate_nodes, inst, penalty_new=0.0, max_suffix_enum=200000):
    seq = []
    rem = set(int(n) for n in candidate_nodes)
    best_route_seq = []
    best_route_sim = {'ok': False}
    while rem:
        nxt = _append_best_stage0_mip(
            int(start_sat), seq, rem, inst, penalty_new=penalty_new, max_suffix_enum=max_suffix_enum
        )
        if nxt is None:
            break
        ok, _, seq_best, sim = _optimize_stage0_route_mip(
            int(start_sat), nxt, inst, penalty_new=penalty_new, max_suffix_enum=max_suffix_enum
        )
        if not ok:
            break
        seq = list(seq_best)
        best_route_seq = list(seq_best)
        best_route_sim = sim
        rem = rem - set(seq)
    return best_route_seq, best_route_sim


def _plan_stage0_routes_mip(inst, active_zlocs, penalty_new=0.0, max_suffix_enum=200000):
    """
    Stage-0 constructor based on the same Stage-MIP route optimizer used in later stages.
    Repeatedly builds one route, removes its covered active customers, and continues until all active
    stage-0 customers are covered.
    """
    active_nodes = set(int(inst['num_dep'] + inst['num_sat'] + int(zloc)) for zloc in active_zlocs)
    S = inst['S']
    selected_routes = []
    ...
    while active_nodes:
        best_route = None
        for start_sat in S:
            seed_seq, sim_seed = _construct_route_stage0_mip(
                int(start_sat), active_nodes, inst, penalty_new=penalty_new, max_suffix_enum=max_suffix_enum
            )
            if not seed_seq:
                continue
            ok, route_cost, seq_best, sim = _optimize_stage0_route_mip(
                int(start_sat), seed_seq, inst, penalty_new=penalty_new, max_suffix_enum=max_suffix_enum
            )
            if not ok:
                continue
            cov = max(1, len(seq_best))
            score = float(route_cost) / float(cov)
            route_info = {
                'start_sat': int(start_sat),
                'seq': [int(z) for z in seq_best],
                'sim': sim if sim.get('ok', False) else sim_seed,
                'route_cost': float(route_cost),
                'score': float(score),
            }
            if (best_route is None or
                    len(seq_best) > len(best_route['seq']) or
                    (len(seq_best) == len(best_route['seq']) and score < best_route['score'] - 1e-9)):
        ...
        active_nodes -= set(int(z) for z in best_route['seq'])
...
def solve_instance_dynamic_pickup(csv_path, w1=5.0, w2=5.0, penalty_new_factor=2.0,
    ...
    ok_stage0, selected_routes, first_str, truck_fixed0, truck_var0, truck_count = _plan_stage0_routes_mip(
        inst, active0, penalty_new=PenaltyNew, max_suffix_enum=max_suffix_enum
    )
    if verbose:
        print(f"[stage0-stageMIP] active customers={len(active0)}, selected routes={len(selected_routes)}, trucks={truck_count}")
    if not ok_stage0:
        return _early_fail_row(0, 'Unsolvable(Stage0-StageMIP)', t0)

    # Build per-drone ownership and stage0 simulation timeline
    num_dep, num_sat = inst["num_dep"], inst["num_sat"]
    Dd, Dp = inst["Dd"], inst["Dp"]
    drone_states = []
    second_routes0 = []
    for didx, route in enumerate(selected_routes):
        ...
        tl = sim.get('timeline', [])
        td0 = float(_stage0_takeoff_time_from_sim(start_sat, seq, sim, inst))
        drone_states.append({
            'id': didx,
            'start_sat': start_sat,
            'prefix_seq': [],
    ...
    # Stage0 objective recompute on heuristic stage0 solution
    ...
    for set_id, name_list in all_sets:
        ...
        for name in name_list:
            ...
            for (w1, w2) in w_pairs:
                detail_xlsx = os.path.join(detail_dir, f"Dynamic_{name}_MIP.xlsx")
...
# =====================
# RH-Stage-MIP overrides
# =====================
def _nearest_sat_dist_from(node, inst):
    _, d = _nearest_sat_from(int(node), inst)
    return float(d)


def _service_h_of_node(z, inst):
    if not _is_customer(int(z), inst):
        return 0.0
    service = inst.get("service", None)
    if service is None or len(service) == 0:
        return 0.0
    zloc = _zloc_from_node(int(z), inst)
    return float(service[zloc]) / 60.0


def _delta_load_of_node(z, inst):
    zloc = _zloc_from_node(int(z), inst)
    return float(inst['Dp'][zloc]) - float(inst['Dd'][zloc])


def _build_best_suffix_order(start_node, start_time_h, start_load, start_E_used, nodes_set, inst,
                             energy_cap=DR['E'], max_enum=200000, ground_wait_free_first=False):
    """
    Stage-MIP suffix solver.
    Preserves the RH-BPC rolling-horizon state construction, but replaces the exact DFS/label-like
    suffix optimizer with a compact MIP over the *current suffix node set* only.

    The MIP enforces:
      - one Hamiltonian path from current state to a return-to-nearest-satellite end,
      - customer time windows,
      - load recursion and capacity,
    and then validates the extracted order by the original exact simulator with payload-dependent energy.

    If the extracted order fails the exact simulator, a no-good cut excludes that order and the MIP is
    resolved. This keeps the physical semantics of RH-BPC unchanged while swapping only the stage solver.
    """
    nodes = sorted(set(int(z) for z in (nodes_set or [])))
    if len(nodes) == 0:
        sim = _simulate_route_from_state(int(start_node), float(start_time_h), float(start_load), float(start_E_used),
                                         [], inst, ground_wait_free=False, remaining_energy_cap=energy_cap)
        return (bool(sim.get('ok', False)), [], sim)
    ...
    dist = inst['dist']
    n = len(nodes)
    node_at = {k + 1: int(nodes[k]) for k in range(n)}
    pos_of = {int(nodes[k]): k + 1 for k in range(n)}

    start_to = {j: float(dist[int(start_node), node_at[j]]) for j in range(1, n + 1)}
    arc = {(i, j): float(dist[node_at[i], node_at[j]]) for i in range(1, n + 1) for j in range(1, n + 1) if i != j}
    end_cost = {i: _nearest_sat_dist_from(node_at[i], inst) for i in range(1, n + 1)}

    due_ub = []
    for j in range(1, n + 1):
        z = node_at[j]
        zloc = _zloc_from_node(z, inst)
        if tw_end is not None and len(tw_end) > zloc and math.isfinite(float(tw_end[zloc])):
            due_ub.append(float(tw_end[zloc]) / 60.0)
    horizon_ub = max(due_ub) if due_ub else float(start_time_h + 24.0)
    M = max(1e4, 10.0 + 60.0 * horizon_ub)

    # limit no-good rounds; keep modest to avoid endless cycling on hard cases
    max_rounds = 12
    order_cuts = []

    best_status = None
    for _round in range(max_rounds):
        m = gp.Model('stage_suffix_mip')
        m.Params.OutputFlag = 0
        m.Params.MIPGap = 0.0
        # use max_enum only as a rough budget hint
        m.Params.TimeLimit = max(5.0, min(60.0, float(max_enum) / 5000.0))
        m.Params.Threads = 0

        x0 = m.addVars(range(1, n + 1), vtype=GRB.BINARY, name='x0')          # start -> j
        xE = m.addVars(range(1, n + 1), vtype=GRB.BINARY, name='xE')          # i -> end(dummy)
        x = m.addVars(list(arc.keys()), vtype=GRB.BINARY, name='x')           # i -> j
        u = m.addVars(range(1, n + 1), lb=1.0, ub=float(n), vtype=GRB.CONTINUOUS, name='u')
        t = m.addVars(range(1, n + 1), lb=0.0, ub=horizon_ub + 24.0, vtype=GRB.CONTINUOUS, name='t')
        l = m.addVars(range(1, n + 1), lb=0.0, ub=float(Q2), vtype=GRB.CONTINUOUS, name='l')

        # degree / path constraints
        m.addConstr(gp.quicksum(x0[j] for j in range(1, n + 1)) == 1, name='start_out')
        m.addConstr(gp.quicksum(xE[i] for i in range(1, n + 1)) == 1, name='end_in')
        for j in range(1, n + 1):
            m.addConstr(x0[j] + gp.quicksum(x[i, j] for i in range(1, n + 1) if i != j) == 1, name=f'in_{j}')
            m.addConstr(xE[j] + gp.quicksum(x[j, k] for k in range(1, n + 1) if k != j) == 1, name=f'out_{j}')
        ...
        for i in range(1, n + 1):
            for j in range(1, n + 1):
                if i == j:
                    continue
                m.addConstr(u[j] >= u[i] + 1 - n * (1 - x[i, j]), name=f'mtz_{i}_{j}')
        for j in range(1, n + 1):
            m.addConstr(u[j] >= 1.0)
            m.addConstr(u[j] <= float(n))
            # if j is first, u[j] = 1
            m.addConstr(u[j] <= 1 + n * (1 - x0[j]))
            m.addConstr(u[j] >= 1 - n * (1 - x0[j]))
        ...
        for j in range(1, n + 1):
            z = node_at[j]
            zloc = _zloc_from_node(z, inst)
            e = float(tw_start[zloc]) / 60.0 if tw_start is not None and len(tw_start) > zloc else 0.0
            l_ub = float(tw_end[zloc]) / 60.0 if tw_end is not None and len(tw_end) > zloc and math.isfinite(float(tw_end[zloc])) else horizon_ub + 24.0
            m.addConstr(t[j] >= e, name=f'tw_lb_{j}')
            m.addConstr(t[j] <= l_ub, name=f'tw_ub_{j}')
            m.addConstr(t[j] >= float(start_time_h) + start_to[j] / max(1e-9, DR['f_s']) - M * (1 - x0[j]), name=f'tstart_{j}')
        for i in range(1, n + 1):
            si = _service_h_of_node(node_at[i], inst)
            for j in range(1, n + 1):
                if i == j:
                    continue
                m.addConstr(t[j] >= t[i] + si + arc[i, j] / max(1e-9, DR['f_s']) - M * (1 - x[i, j]), name=f'tprop_{i}_{j}')

        # load propagation
        for j in range(1, n + 1):
            dj = _delta_load_of_node(node_at[j], inst)
            m.addConstr(l[j] <= float(start_load) + dj + M * (1 - x0[j]), name=f'llb0u_{j}')
            m.addConstr(l[j] >= float(start_load) + dj - M * (1 - x0[j]), name=f'llb0l_{j}')
        for i in range(1, n + 1):
            for j in range(1, n + 1):
                if i == j:
                    continue
                dj = _delta_load_of_node(node_at[j], inst)
                m.addConstr(l[j] <= l[i] + dj + M * (1 - x[i, j]), name=f'llin_u_{i}_{j}')
                m.addConstr(l[j] >= l[i] + dj - M * (1 - x[i, j]), name=f'llin_l_{i}_{j}')

        # exact start-load capacity should already hold, but keep explicit guard
        if float(start_load) > float(Q2) + 1e-9 or float(start_load) < -1e-9:
            return (False, [], {'ok': False, 'reason': 'bad_start_load'})

        # no-good cuts for previously rejected complete orders (arc-based)
        for cut_id, seq in enumerate(order_cuts):
            if not seq:
                continue
            expr = x0[pos_of[seq[0]]]
            for a, b in zip(seq[:-1], seq[1:]):
                expr += x[pos_of[a], pos_of[b]]
            expr += xE[pos_of[seq[-1]]]
            m.addConstr(expr <= n, name=f'nogood_{cut_id}')

        # objective: route distance proxy + mild lateness discouragement
        obj = gp.quicksum(start_to[j] * x0[j] for j in range(1, n + 1)) + \
              gp.quicksum(arc[i, j] * x[i, j] for (i, j) in arc) + \
              gp.quicksum(end_cost[i] * xE[i] for i in range(1, n + 1)) + \
              1e-4 * gp.quicksum(t[j] for j in range(1, n + 1))
        m.setObjective(obj, GRB.MINIMIZE)
        m.optimize()
        best_status = int(m.Status)

        if m.SolCount <= 0:
            return (False, [], {'ok': False, 'mip_status': best_status})

        # extract one Hamiltonian path order
        first = None
        for j in range(1, n + 1):
            if x0[j].X > 0.5:
                first = j
                break
        if first is None:
            return (False, [], {'ok': False, 'reason': 'no_first'})
        ...
        while True:
            ...
            cur = nxt
        if len(order_idx) != n:
            return (False, [], {'ok': False, 'reason': 'bad_path_extract', 'mip_status': best_status})

        seq = [node_at[k] for k in order_idx]
        sim = _simulate_route_from_state(int(start_node), float(start_time_h), float(start_load), float(start_E_used),
                                         seq, inst,
                                         ground_wait_free=bool(ground_wait_free_first),
                                         remaining_energy_cap=energy_cap)
        ...
        # forbid this exact order and retry
        order_cuts.append(tuple(seq))

    return (False, [], {'ok': False, 'reason': 'all_mip_orders_failed_sim', 'mip_status': best_status})
...
def _find_best_new_pickup_only_sortie(remaining_pick_nodes, inst, t_dec_h, max_suffix_enum,
                                     ...
                                     allowed_start_sats=None):
    """
    Stage-MIP new pickup-only sortie constructor.
    Keeps RH-BPC's multi-new-sortie logic unchanged, but swaps the internal subset-order solver
    from DFS to the MIP-based suffix solver above.
    """
    rem = sorted(set(int(z) for z in remaining_pick_nodes))
    if not rem:
        return None
    ...
    def _eval_subset(s0, subset_nodes):
        ...
        return {
            ...
            'sim': sim_best,
            'covered_nodes': set(int(z) for z in subset_nodes),
            'end_energy_used': float(sim_best.get('end_energy_used', float('inf')))
        }

    best = None

    # try larger subsets first
    Kmax = min(len(rem), int(exact_subset_limit))
    for k in range(Kmax, 0, -1):
        tried = 0
        if len(rem) <= exact_subset_limit:
            subs_iter = combinations(rem, k)
        else:
            # simple priority shortlist by earlier due date, then nearer to each possible start sat
            base = sorted(rem, key=lambda z: ((float(tw_end[_zloc_from_node(z, inst)]) if tw_end is not None else 1e18), int(z)))
            shortlist = base[:min(len(base), max(6, 2 * k + 4))]
            subs_iter = combinations(shortlist, k)
        for subset in subs_iter:
            subset_nodes = tuple(int(z) for z in subset)
            for s0 in start_sat_candidates:
                cand = _eval_subset(s0, subset_nodes)
                if _better(cand, best):
                    best = cand
            tried += 1
            if len(rem) > exact_subset_limit and tried >= int(max_subset_trials_per_k):
                break
        if best is not None and len(best['covered_nodes']) == k:
            return best

    # fallback: single-pickup best
    for z in rem:
        for s0 in start_sat_candidates:
            cand = _eval_subset(s0, (int(z),))
            if _better(cand, best):
                best = cand
    return best
...
DYNAMIC_OUT_XLSX = "DynamicPickup_results_Ca1-3,5,50-StageMIP.xlsx"
