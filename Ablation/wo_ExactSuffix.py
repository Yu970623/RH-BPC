# -*- coding: utf-8 -*-
# Ablation variant: w/o exact suffix optimization (queue-aware checked version)
import os
import math
import time
import numpy as np
...
Target_DIR = r"D:\OneDrive-CSU\OneDrive - csu.edu.cn\2E-VRP\RH-BCP\Experiment\Ablation\no_Exact"
TIME_LIMIT = 3600
MIP_GAP = 0.0
OUTPUT_LOG = 1
...
def _best_feasible_insertion_from_state(start_node, start_time_h, start_load, start_E_used,
                                        base_seq, add_node, inst,
                                        energy_cap=DR['E'], ground_wait_free_first=False):
    """
    Greedy local insertion: insert add_node into the current sequence at the best feasible position.
    This is a heuristic replacement for exact suffix enumeration.
    Returns (ok, best_seq, best_sim).
    """
    base_seq = [int(z) for z in (base_seq or [])]
    add_node = int(add_node)
    best = None  # (E_end, t_end, seq, sim)
    for pos in range(len(base_seq) + 1):
        cand = base_seq[:pos] + [add_node] + base_seq[pos:]
        ...
        if not sim.get('ok', False):
            continue
        E_end = float(sim.get('end_energy_used', float('inf')))
        t_end = float(sim.get('end_time_h', float('inf')))
        rec = (E_end, t_end, cand, sim)
        if (best is None) or (E_end < best[0] - 1e-9) or (abs(E_end - best[0]) <= 1e-9 and t_end < best[1] - 1e-9):
            best = rec
    if best is None:
        return (False, [], {'ok': False})
    return (True, [int(z) for z in best[2]], best[3])



def _build_suffix_order_heuristic(start_node, start_time_h, start_load,
                                  start_E_used=None, nodes_set=None, inst=None,
                                  energy_cap=DR['E'], order_seed=None,
                                  ground_wait_free_first=False,
                                  start_energy_used=None, max_enum=None):
    """
    Backward-compatible heuristic suffix constructor:
    accepts both start_E_used and start_energy_used;
    ignores legacy max_enum if passed.
    """
    if start_E_used is None:
        start_E_used = start_energy_used
    if start_E_used is None:
        start_E_used = 0.0

    nodes_set = set(int(z) for z in (nodes_set or []))

    target = []
    seen = set()
    for z in (order_seed or []):
        zi = int(z)
        if zi in nodes_set and zi not in seen:
            target.append(zi)
            seen.add(zi)

    tw_start = inst.get('tw_start', None)
    dist = inst['dist']
    remaining = [int(z) for z in nodes_set if int(z) not in seen]

    def _prio_key(z):
        zloc = _zloc_from_node(int(z), inst)
        tws = float(tw_start[zloc]) if tw_start is not None and len(tw_start) > zloc else 0.0
        return (tws, float(dist[int(start_node), int(z)]), int(zloc))

    remaining.sort(key=_prio_key)
    ...
    for z in target:
        ok_ins, seq_ins, sim_ins = _best_feasible_insertion_from_state(
            start_node=int(start_node),
            start_time_h=float(start_time_h),
            start_load=float(start_load),
            ...
            ground_wait_free_first=bool(ground_wait_free_first)
        )
        if not ok_ins:
            return (False, [], {'ok': False})
        cur_seq = [int(v) for v in seq_ins]
        cur_sim = sim_ins

    return (True, [int(z) for z in cur_seq], cur_sim)
...
def _build_suffix_order_heuristic(start_node, start_time_h, start_load, start_E_used, nodes_set, inst,
                                  energy_cap=DR['E'], order_seed=None, ground_wait_free_first=False):
    """
    Heuristic suffix constructor without exact enumeration.
    Strategy:
      1) build a priority list that first respects the old remaining order (order_seed),
         then appends all other uncovered nodes sorted by TW start / distance;
      2) insert nodes one-by-one into the current route at the best feasible position.
    This keeps the rolling-horizon logic but removes exact suffix optimization.
    """
    target = []
    seen = set()
    for z in (order_seed or []):
    ...
    for idx, z in enumerate(target):
        ok, seq, sim = _best_feasible_insertion_from_state(
            start_node=int(start_node),
            start_time_h=float(start_time_h),
            start_load=float(start_load),
            start_energy_used=float(start_E_used),
        ...
        if not ok:
            return (False, [], {'ok': False, 'failed_node': int(z), 'failed_idx': int(idx)})

    return (True, [int(z) for z in seq], sim)
...
def _find_best_new_pickup_only_sortie(remaining_pick_nodes, inst, t_dec_h, max_suffix_enum,
    ...
    def _eval_subset(s0, subset_nodes):
        ok, seq_best, sim_best = _build_suffix_order_heuristic(
            start_node=int(s0),
...
def solve_instance_dynamic_pickup(csv_path, w1=5.0, w2=5.0, penalty_new_factor=2.0,
    ...
    def _znode_stage0(zloc):
        return int(inst["num_dep"] + inst["num_sat"] + int(zloc))

    def _node_to_zloc_stage0(node):
        return _zloc_from_node(int(node), inst)

    def _all_active_nodes_stage0(active_zlocs):
        return [_znode_stage0(zloc) for zloc in active_zlocs]

    def _sequence_delivery_load_stage0(seq_nodes):
        total = 0
        for node in seq_nodes:
            zloc = _node_to_zloc_stage0(int(node))
            total += int(inst["Dd"][zloc])
        return int(total)

    def _stage0_takeoff_time_from_sim(start_sat, seq_nodes, sim):
        if not seq_nodes:
            return 0.0
    ...
    def _first_echelon_direct_trips_stage0(selected_routes):
        P = inst['P']
        S = inst['S']
        dist = inst['dist']
        ...
        for route in selected_routes:
            ...
            sat_delivery[s] += float(_sequence_delivery_load_stage0(route.get('seq', [])))
    ...
    def _stage0_route_cost_from_sim(seq_nodes, sim):
        delivery_load = _sequence_delivery_load_stage0(seq_nodes)
        fixed_cost = float(h2) + (float(PenaltyNew) if delivery_load == 0 else 0.0)
        variable_cost = float(c_u) * float(sim.get('end_energy_used', 0.0))
        return float(fixed_cost + variable_cost)


    def _stage0_optimize_node_set(start_sat, node_set):
        node_set = set(int(z) for z in node_set)
        if not node_set:
            return False, [], {"ok": False}
        start_load0 = float(_sequence_delivery_load_stage0(node_set))
        ok, seq_best, sim_best = _build_suffix_order_heuristic(
            start_node=int(start_sat),
            start_time_h=0.0,
            start_load=float(start_load0),
            start_energy_used=0.0,
            nodes_set=set(int(z) for z in node_set),
            inst=inst,
            energy_cap=DR['E'],
            ground_wait_free_first=True,
        )
        return bool(ok), [int(z) for z in seq_best], sim_best


    def _construct_route_stage0(start_sat, candidate_nodes):
        current_set = set()
        current_cost = 0.0
        best_seq = []
        best_sim = {'ok': False}
        remaining = set(int(n) for n in candidate_nodes)
        while remaining:
            chosen = None
            cand_list = list(remaining)
            cand_list.sort(key=lambda n: (_stage0_sort_key(n), float(inst['dist'][int(start_sat), int(n)])))
            for node in cand_list:
                trial_set = set(current_set)
                trial_set.add(int(node))
                ok, seq_best, sim_best = _stage0_optimize_node_set(int(start_sat), trial_set)
                if not ok:
                    continue
                route_cost = _stage0_route_cost_from_sim(seq_best, sim_best)
                incr = float(route_cost) - (float(current_cost) if current_set else 0.0)
                zloc = _node_to_zloc_stage0(int(node))
                tw_end = inst.get('tw_end', None)
                ...
                if (chosen is None or score < chosen['score'] - 1e-12 or
                        (abs(score - chosen['score']) <= 1e-12 and len(cand['seq']) > len(chosen['seq']))):
                    chosen = cand
            if chosen is None:
                break
            current_set = set(int(z) for z in chosen['seq'])
            best_seq = [int(z) for z in chosen['seq']]
            best_sim = chosen['sim']
            current_cost = float(chosen['route_cost'])
            remaining -= current_set
        return best_seq, best_sim, float(current_cost)

    def _plan_stage0_routes():
        active_nodes = set(_all_active_nodes_stage0(active0))
        selected_routes = []
        S = inst['S']

        if not active_nodes:
            first_str0, truck_fixed_cost0, truck_var_cost0, truck_cnt0 = _first_echelon_direct_trips_stage0([])
            return True, [], first_str0, truck_fixed_cost0, truck_var_cost0, truck_cnt0

        while active_nodes:
            best_route = None
            for start_sat in S:
                seq, sim, route_cost = _construct_route_stage0(int(start_sat), active_nodes)
                if not seq:
                    continue
                cov = max(1, len(seq))
                ...
                route_info = {
                    ...
                    'seq': [int(z) for z in seq],
                    'sim': sim,
                    'route_cost': float(route_cost),
                    'score': float(score),
                }
                if (best_route is None or
                        len(seq) > len(best_route['seq']) or
                        (len(seq) == len(best_route['seq']) and score < best_route['score'] - 1e-9)):
        ...
        first_str0, truck_fixed_cost0, truck_var_cost0, truck_cnt0 = _first_echelon_direct_trips_stage0(selected_routes)
        return True, selected_routes, first_str0, truck_fixed_cost0, truck_var_cost0, truck_cnt0

    ok_stage0, selected_routes, first_str, truck_fixed0, truck_var0, truck_count = _plan_stage0_routes()
    if verbose:
        print(f"[stage0-heuristic] active customers={len(active0)}, selected routes={len(selected_routes)}, trucks={truck_count}")
    if not ok_stage0:
        return _early_fail_row(0, 'Unsolvable(Stage0-HeuristicSuffix)', t0)
    ...
    for didx, route in enumerate(selected_routes):
        ...
        if sim is None or not sim.get('ok', False):
            ok_tmp, seq_tmp, sim_tmp = _stage0_optimize_node_set(start_sat, set(seq))
            if not ok_tmp:
                return _early_fail_row(0, f'Stage0RouteSimFail(d{didx})', t0)
            seq = [int(z) for z in seq_tmp]
            sim = sim_tmp
        if not sim.get('ok', False):
            return _early_fail_row(0, f'Stage0RouteSimFail(d{didx})', t0)
        ...
        drone_states.append({
            ...
            'tD': float(_stage0_takeoff_time_from_sim(start_sat, seq, sim)),
        })
    ...
    drone_fixed0 = float(h2) * float(len(selected_routes))
    penalty_part0 = 0.0
    drone_var0 = 0.0
    for route in selected_routes:
        seq = route.get('seq', [])
        sim = route.get('sim', {})
        if _sequence_delivery_load_stage0(seq) == 0:
            penalty_part0 += float(PenaltyNew)
        drone_var0 += float(c_u) * float(sim.get('end_energy_used', 0.0))

    obj1_0 = float(truck_fixed0) + float(drone_fixed0) + float(penalty_part0)
    obj2_0 = float(truck_var0) + float(drone_var0)
    ...
    while pending:
        ...
        for st in per_drone_state:
            ...
            if not ok:
                ok, best_seq, best_sim = _build_suffix_order_heuristic(
                    start_node=st["cur_node"],
                    start_time_h=st["cur_time_h"],
                    start_load=st["cur_load"],
                    start_energy_used=st["cur_E"],
    ...
    for set_id, name_list in all_sets:
        ...
        for name in name_list:
            ...
            for (w1, w2) in w_pairs:
                detail_xlsx = os.path.join(detail_dir, f"Dynamic_{name}_no_Exact.xlsx")
...
# ================= Compatibility override block (auto-fixed) =================
# These definitions intentionally override earlier mixed old/new interfaces so that
# the ablation variant can run without keyword-mismatch errors.
_old__simulate_route_from_state = _simulate_route_from_state

def _simulate_route_from_state(start_node, start_time_h, start_load, start_energy_used=None,
                               seq_customers=None, inst=None, ground_wait_free=False,
                               remaining_energy_cap=None, start_E_used=None, **kwargs):
    if start_energy_used is None:
        start_energy_used = start_E_used
    if start_energy_used is None:
        start_energy_used = 0.0
    return _old__simulate_route_from_state(
        start_node, start_time_h, start_load, start_energy_used,
        seq_customers, inst, ground_wait_free, remaining_energy_cap
    )

def _best_feasible_insertion_from_state(start_node, start_time_h, start_load,
                                        start_E_used=None, base_seq=None, add_node=None, inst=None,
                                        energy_cap=DR['E'], ground_wait_free_first=False,
                                        start_energy_used=None, **kwargs):
    if start_E_used is None:
        start_E_used = start_energy_used
    if start_E_used is None:
        start_E_used = 0.0
...
def _build_suffix_order_heuristic(start_node, start_time_h, start_load,
                                  ...
                                  energy_cap=DR['E'], order_seed=None,
                                  ground_wait_free_first=False,
                                  start_energy_used=None, max_enum=None, **kwargs):
    # Backward-compatible heuristic suffix constructor:
    # accepts both start_E_used and start_energy_used; ignores max_enum.
    if start_E_used is None:
        start_E_used = start_energy_used
    if start_E_used is None:
        start_E_used = 0.0
    ...
    return (True, [int(z) for z in seq], sim)
# ================= End compatibility override block =================
# ------------------ Quick switch ------------------
DYNAMIC_MODE = False
DYNAMIC_BATCH_MODE = True
DYNAMIC_CSV_NAME = "Ca1-3,5,50.csv"
DYNAMIC_OUT_XLSX = "DynamicPickup_results_Ca1-3,5,50-noExact.xlsx"
