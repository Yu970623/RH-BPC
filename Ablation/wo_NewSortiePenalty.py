...
def solve_instance_dynamic_pickup(csv_path, w1=5.0, w2=5.0, penalty_new_factor=0.0,
    ...
    if verbose:
        print(f"[stage0-exactsuffix] active customers={len(active0)}, selected routes={len(selected_routes)}, trucks={truck_count}")
    if not ok_stage0:
        return _early_fail_row(0, 'Unsolvable(Stage0-ExactSuffix)', t0)
...
def run_dynamic_single(csv_path, out_xlsx=None, w1=5.0, w2=5.0, penalty_new_factor=0.0,
...
def run_dynamic_batch(all_sets, detail_dir_name="DynamicPickup_details_wo_NewSortiePenalty",
                      w_pairs=None, penalty_new_factor=0.0, max_suffix_enum=200000, lock_unaffected=True):
    if w_pairs is None:
        w_pairs = [(5.0, 5.0)]
    ...
    for set_id, name_list in all_sets:
        ...
        for name in name_list:
            ...
            for (w1, w2) in w_pairs:
                detail_xlsx = os.path.join(detail_dir, f"Dynamic_{name}_NewSortiePenalty.xlsx")
...
DYNAMIC_OUT_XLSX = "DynamicPickup_results_wo_NewSortiePenalty.xlsx"
if __name__ == "__main__":
    if DYNAMIC_BATCH_MODE:
        ...
        run_dynamic_single(csv_path, out_xlsx=out_xlsx, w1=5.0, w2=5.0,
                           penalty_new_factor=0.0, max_suffix_enum=200000, lock_unaffected=True)
    else:
        run_batch()
