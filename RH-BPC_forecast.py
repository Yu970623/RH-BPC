...
Target_DIR = r"D:\OneDrive-CSU\OneDrive - csu.edu.cn\2E-VRP\RH-BCP\Experiment\RH-BCP v3"
TIME_LIMIT = 3600
MIP_GAP = 0.0
OUTPUT_LOG = 1
...
# ===== Perfect-forecast control =====
# If True, all future pickup requests are assumed to be perfectly known at Stage0.
# The rolling-horizon execution, prefix freezing, and suffix optimization are preserved,
# but stage planning is anticipative because unreleased pickups are allowed to remain in
# the optimized suffix from the beginning.
PERFECT_FORECAST_MODE = False

# ===== Forecast-perturbed anticipative mode =====
# This mode is designed for the subsection:
#   "Value of perfect forecast information in dynamic replanning"
# Logic:
#   - future pickup requests are NOT perfectly known;
#   - instead, unreleased pickups are represented by a forecast scenario obtained by
#     perturbing the true future pickup weight and time-window parameters;
#   - the stage model still optimizes over all current + forecasted future pickups (strong reservation),
#     but when a pickup is actually released, its forecast attributes are replaced by the true ones.
# This produces a prediction-informed, non-perfect benchmark that sits between:
#   RH-BPC (no future information) and perfect forecast (all future information exact).
FORECAST_PERTURBED_MODE = True

# Conservative perturbation settings for forecasted future pickups
PF_FORECAST_RANDOM_SEED = 20250424
PF_FORECAST_DEMAND_UPLIFT_MAX_RATIO = 0.30   # forecasted pickup weight >= true weight
PF_FORECAST_TW_START_DELAY_MAX_MIN = 10.0    # predicted start is shifted later (or equal)
PF_FORECAST_TW_END_SHRINK_MAX_MIN = 15.0     # predicted end is shifted earlier (or equal)
PF_FORECAST_MIN_TW_WIDTH_MIN = 20.0          # keep predicted TW feasible
...
def _stable_seed_from_path(csv_path_or_name):
    s = str(csv_path_or_name)
    val = 0
    for ch in s:
        val = (val * 131 + ord(ch)) % (2**31 - 1)
    return int((PF_FORECAST_RANDOM_SEED + val) % (2**31 - 1))


def _prepare_forecast_arrays(inst, release_min, seed_key=""):
    """Build a conservative forecast scenario for future pickups.
    Future pickups are perturbed only in:
      - pickup weight (forecast >= true)
      - time window start / end (forecast TW is equal or tighter than true)
    This keeps the strong-reservation effect while avoiding the subjectivity of ad-hoc soft penalties.
    """
    rng = np.random.default_rng(_stable_seed_from_path(seed_key))
    num_cus = inst["num_cus"]

    Dp_true = np.array(inst["Dp"], dtype=float).copy()
    tws_true = np.array(inst.get("tw_start", np.zeros(num_cus, dtype=float)), dtype=float).copy()
    twe_true = np.array(inst.get("tw_end", np.zeros(num_cus, dtype=float)), dtype=float).copy()

    Dp_fc = Dp_true.copy()
    tws_fc = tws_true.copy()
    twe_fc = twe_true.copy()

    for zloc in range(num_cus):
        if int(Dp_true[zloc]) <= 0:
            continue
        if float(release_min[zloc]) <= 1e-9:
            # already released at stage0 => keep actual values
            continue

        # Conservative weight uplift: reserve at least as much payload as true demand
        uplift = float(rng.uniform(0.0, float(PF_FORECAST_DEMAND_UPLIFT_MAX_RATIO)))
        Dp_fc[zloc] = min(float(Q2), float(Dp_true[zloc]) * (1.0 + uplift))

        # Conservative TW perturbation: start no earlier than truth, end no later than truth
        d_start = float(rng.uniform(0.0, float(PF_FORECAST_TW_START_DELAY_MAX_MIN)))
        d_end = float(rng.uniform(0.0, float(PF_FORECAST_TW_END_SHRINK_MAX_MIN)))

        tws_hat = float(tws_true[zloc]) + d_start
        twe_hat = float(twe_true[zloc]) - d_end

        if twe_hat < tws_hat + float(PF_FORECAST_MIN_TW_WIDTH_MIN):
            twe_hat = tws_hat + float(PF_FORECAST_MIN_TW_WIDTH_MIN)

        # Never make the forecast less conservative than the true TW
        tws_fc[zloc] = max(float(tws_true[zloc]), tws_hat)
        twe_fc[zloc] = min(float(twe_true[zloc]), twe_hat) if float(twe_true[zloc]) > 0 else twe_hat
        if twe_fc[zloc] < tws_fc[zloc] + 1e-9:
            twe_fc[zloc] = tws_fc[zloc] + float(PF_FORECAST_MIN_TW_WIDTH_MIN)

    inst["_Dp_true"] = Dp_true
    inst["_tw_start_true"] = tws_true
    inst["_tw_end_true"] = twe_true
    inst["_Dp_forecast"] = Dp_fc
    inst["_tw_start_forecast"] = tws_fc
    inst["_tw_end_forecast"] = twe_fc


def _apply_forecast_mixture_inplace(inst, release_min, t_cut_min):
    """For released pickups use true attributes; for unreleased pickups use forecast attributes.
    This creates a prediction-informed strong-reservation stage model while keeping release semantics:
    once a pickup is actually released, the stage model immediately switches to the true demand/TW.
    """
    if not bool(FORECAST_PERTURBED_MODE):
        return inst

    num_cus = inst["num_cus"]
    Dp_mix = np.array(inst["_Dp_true"], dtype=float).copy()
    tws_mix = np.array(inst["_tw_start_true"], dtype=float).copy()
    twe_mix = np.array(inst["_tw_end_true"], dtype=float).copy()

    for zloc in range(num_cus):
        if float(inst["_Dp_true"][zloc]) <= 0:
            continue
        if float(release_min[zloc]) > float(t_cut_min) + 1e-9:
            # unreleased => stage sees forecasted version
            Dp_mix[zloc] = float(inst["_Dp_forecast"][zloc])
            tws_mix[zloc] = float(inst["_tw_start_forecast"][zloc])
            twe_mix[zloc] = float(inst["_tw_end_forecast"][zloc])

    inst["Dp"] = Dp_mix
    inst["tw_start"] = tws_mix
    inst["tw_end"] = twe_mix
    return inst
...
def _active_zlocs_stage0_perfect_forecast(inst):
    """Perfect-forecast mode: all deliveries and all pickups are known at Stage0."""
    Dd = inst["Dd"]
    Dp = inst["Dp"]
    num_cus = inst["num_cus"]
    active = []
    for zloc in range(num_cus):
        if int(Dd[zloc]) > 0 or int(Dp[zloc]) > 0:
            active.append(zloc)
    return sorted(active)
...
def solve_instance_dynamic_pickup(csv_path, w1=5.0, w2=5.0, penalty_new_factor=2.0,
    ...
    """
    Main entry for the dynamic-pickup requirement.
    Returns a list of per-stage result dicts (each dict is a table row).
    If PERFECT_FORECAST_MODE=True, all future pickup requests are assumed known exactly at Stage0.
    If FORECAST_PERTURBED_MODE=True, all future pickups are represented by a perturbed forecast scenario,
    while rolling execution, prefix freezing, and suffix optimization are still preserved.
    """
    ...
    # prepare forecast scenario (future pickups only)
    if FORECAST_PERTURBED_MODE:
        _prepare_forecast_arrays(inst, release_min, seed_key=os.path.basename(csv_path))
        _apply_forecast_mixture_inplace(inst, release_min, 0.0)
    ...
    # stage0 active set
    # - normal mode: deliveries + pickups with release==0
    # - perfect-forecast mode: all deliveries + all pickups are known at Stage0
    if PERFECT_FORECAST_MODE or FORECAST_PERTURBED_MODE:
        active0 = _active_zlocs_stage0_perfect_forecast(inst)
    else:
    ...
    def _early_fail_row(stage_id: int, status: str, t_start: float, **extra):
        row = {
            ...
            "ForecastMode": "PerfectForecast" if PERFECT_FORECAST_MODE else ("ForecastPerturbed" if FORECAST_PERTURBED_MODE else "Online"),
        }
        if extra:
            row.update(extra)
        return [row]
    ...
    pool_cols = sum(len(routes_by_s[s]) for s in routes_by_s)
    if verbose:
        print(f"[stage0-pool] active customers={len(active0)}, pool_cols={pool_cols}, global_opt={STAGE0_REQUIRE_GLOBAL_OPT}, perfect_forecast={PERFECT_FORECAST_MODE}, forecast_perturbed={FORECAST_PERTURBED_MODE}")
    ...
    # - active_pick_nodes: pickup customer nodes whose request has actually been released
    # - all_known_pick_nodes: all pickup customer nodes known to the perfect-forecast oracle
    # - served_pick_nodes: pickup customer nodes already completed (t_end <= current decision time)
    active_pick_nodes = set()
    all_known_pick_nodes = set()
    served_pick_nodes = set()
    for _zloc in range(inst["num_cus"]):
        if int(Dp[_zloc]) > 0:
            _node = int(inst["num_dep"] + inst["num_sat"] + _zloc)
            all_known_pick_nodes.add(_node)
            if float(release_min[_zloc]) <= 1e-9:
                active_pick_nodes.add(_node)
    ...
    def _log_pick_visible(node, t_cut_min):
        try:
            node = int(node)
        except Exception:
            return False
        ...
        if int(Dp[zloc]) <= 0:
            return True
        return float(release_min[zloc]) <= float(t_cut_min) + 1e-9

    def _visible_seq_for_logging(seq, t_cut_min):
        out = []
        for _n in seq:
            _n = int(_n)
            if (not _is_customer(_n, inst)):
                continue
            zloc = _zloc_from_node(_n, inst)
            if int(Dd[zloc]) > 0 or float(release_min[zloc]) <= float(t_cut_min) + 1e-9:
                out.append(_n)
        return out

    def _visible_energy_from_timeline(tl, t_cut_min):
        e = 0.0
        for rec in tl:
            try:
                _n = int(rec.get("node"))
                ...
                continue
            if _log_pick_visible(_n, t_cut_min):
                e = float(rec.get("E_used", e))
        return float(e)

    def _visible_plan_stats(dr_states, t_cut_min):
        parts = []
        total_e = 0.0
        vis_count = 0
        vis_new = 0
        for _ds in dr_states:
            seq_vis = _visible_seq_for_logging(_ds.get("plan_seq", []), t_cut_min)
            if not seq_vis:
                continue
            vis_count += 1
            if int(_ds.get("created_stage", 0)) > 0:
                vis_new += 1
            s0 = int(_ds.get("start_sat", 0))
            end_sat0 = int(_nearest_sat_from(seq_vis[-1], inst)[0]) if len(seq_vis) > 0 else s0
            parts.append(_format_route_12(s0, seq_vis, end_sat0, inst))
            total_e += _visible_energy_from_timeline(_ds.get("timeline", []), t_cut_min)
        return " | ".join(parts), float(total_e), int(vis_count), int(vis_new)

    stage_rows = []
    second_str_log0, total_energy_log0, vis_count0, vis_new0 = _visible_plan_stats(drone_states, 0.0) if (PERFECT_FORECAST_MODE or FORECAST_PERTURBED_MODE) else (second_str_fmt0, sum(float(d.get("end_energy_used", 0.0)) for d in drone_states), len(drone_states), 0)
    obj1_log0 = float(truck_fixed0) + float(h2) * float(vis_count0) + float(PenaltyNew) * float(vis_new0)
    obj2_log0 = float(truck_var0) + float(c_u) * float(total_energy_log0)
    stage_rows.append({
        "Instances": os.path.basename(csv_path),
        "Stage": 0,
        ...
        "Second-echelon": second_str_log0,
        "DecisionTimes(min)": "",
        "NumSorties": len(drone_states),
        "Obj1": round(obj1_log0, 6),
        "Obj2": round(obj2_log0, 6),
        "Sum_obj": round(obj1_log0 + obj2_log0, 6),
        "PenaltyNew": round(PenaltyNew, 6),
        "NewSortieCreated": 0,
        "DroneEnergy(Wh)": round(float(total_energy_log0), 6),
    ...
    while pending:
        ...
        # Update the stage model seen by the planner:
        # released pickups use true attributes; unreleased pickups use forecast attributes.
        if FORECAST_PERTURBED_MODE:
            _apply_forecast_mixture_inplace(inst, release_min, t_dec_min_f)
            tw_end_arr = inst.get("tw_end", None)
        ...
        for info in drone_tmp:
            ds = info["ds"]
            did = int(ds["id"])
            prev_picks = set(ds.get("pickup_assigned", set())) - set(served_nodes)
        ...
        if PERFECT_FORECAST_MODE:
            second_str_stage, total_energy_stage, vis_count_stage, vis_new_stage = _visible_plan_stats(drone_states, t_dec_min_f)
            drone_fixed = float(h2) * float(vis_count_stage) + float(PenaltyNew) * float(vis_new_stage)
        else:
            second_routes = []
            for ds in drone_states:
...
DYNAMIC_OUT_XLSX = "DynamicPickup_results_Ca1-3,5,50-BPC-PerfectForecast.xlsx"
