%% Generate strict-format Excel tables with time statistics
% This script reads raw result files of RH-BPC / Greedy / MIP / Repair
% and outputs strict-format Excel tables matching the user's layout.
%
% Main design choice for "Mean Time":
%   -> mean TOTAL solve time per instance
%      = sum of stage-wise solve times over all stages of one instance,
%      then averaged across instances in the same scale group.
%
% Rationale:
%   1) It best reflects the true computational burden of solving one full dynamic instance.
%   2) All compared algorithms use the same event-triggered stage partition, so total instance
%      time is directly comparable.
%   3) Mean stage time can artificially dilute the exact method's Stage-0 burden.
%
% The script also exports an auxiliary sheet with:
%   - Mean Total Time per Instance
%   - Mean Stage Time
%   - Mean Number of Stages
%
% Output workbook:
...
for g = 1:numel(groupOrder)
    ...
    for a = 1:numel(algOrder)
        ...
        if isempty(Ta)
            ...
            meanTime  = mean(Ta.TotalTimePerInstance, 'omitnan'); % main metric
        end

        if a == 1
            row = {grp, groupN(g), algOrder{a}, numfmt(meanInit,2), numfmt(meanFinal,2), numfmt(meanDelta,2), ...
                   pairInit(algOrder{a}), pairFinal(algOrder{a}), pairDelta(algOrder{a}), numfmt(meanTime,3)};
        else
