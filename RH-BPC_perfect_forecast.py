...
Target_DIR = r"D:\OneDrive-CSU\OneDrive - csu.edu.cn\2E-VRP\RH-BCP\Experiment\RH-BCP v2"
TIME_LIMIT = 3600
MIP_GAP = 0.0
OUTPUT_LOG = 1
...
def solve_instance_dynamic_pickup(csv_path, w1=5.0, w2=5.0, penalty_new_factor=2.0,
    ...
    If PERFECT_FORECAST_MODE=True, all future pickup requests are assumed known at Stage0,
    ...
    def _early_fail_row(stage_id: int, status: str, t_start: float, **extra):
        row = {
            ...
            "ForecastMode": "PerfectForecast" if PERFECT_FORECAST_MODE else "Online",
        }
        if extra:
            row.update(extra)
        return [row]
    ...
    pool_cols = sum(len(routes_by_s[s]) for s in routes_by_s)
    if verbose:
        print(f"[stage0-pool] active customers={len(active0)}, pool_cols={pool_cols}, global_opt={STAGE0_REQUIRE_GLOBAL_OPT}, perfect_forecast={PERFECT_FORECAST_MODE}")
    ...
    stage_rows = []
    second_str_log0, total_energy_log0, vis_count0, vis_new0 = _visible_plan_stats(drone_states, 0.0) if PERFECT_FORECAST_MODE else (second_str_fmt0, sum(float(d.get("end_energy_used", 0.0)) for d in drone_states), len(drone_states), 0)
